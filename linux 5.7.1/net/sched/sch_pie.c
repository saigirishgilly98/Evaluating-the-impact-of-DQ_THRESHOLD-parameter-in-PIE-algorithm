...
static const struct nla_policy pie_policy[TCA_PIE_MAX + 2] = {
	...
	[TCA_PIE_DQ_THRESH]		= {.type = NLA_U32},
};
...
{
	struct pie_sched_data *q = qdisc_priv(sch);
	struct nlattr *tb[TCA_PIE_MAX + 2];
	...
	if (tb[TCA_PIE_DQ_THRESH])
		q->params.dq_thresh = nla_get_u32(tb[TCA_PIE_DQ_THRESH]);		
			 ...
			 struct pie_vars *vars, u32 backlog)
{
	 ...
	 */
	if (backlog >= (params->dq_thresh) && vars->dq_count == DQCOUNT_INVALID) {
	...
	if (vars->dq_count != DQCOUNT_INVALID) {
		...
		if (vars->dq_count >= (params->dq_thresh)) {
...
static int pie_dump(struct Qdisc *sch, struct sk_buff *skb)
{
	...
	if (nla_put_u32(skb, TCA_PIE_TARGET,
	    ...
	    nla_put_u32(skb, TCA_PIE_DQ_RATE_ESTIMATOR,
			q->params.dq_rate_estimator) ||
	    nla_put_u32(skb, TCA_PIE_DQ_THRESH, q->params.dq_thresh))
		goto nla_put_failure;
